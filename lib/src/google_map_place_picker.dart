...
class GoogleMapPlacePicker extends StatelessWidget {
  ...
  bool _checkIfValidArea(LatLng tap) {
    if (polygons != null) {
      int intersectCount = 0;
      var points = polygons.values.toList();
      for (var p in points) {
        for (int j = 0; j < p.points.length - 1; j++) {
          if (rayCastIntersect(tap, p.points[j], p.points[j + 1])) {
            intersectCount++;
          }
        }
      }
  ...
  _searchByCameraLocation(PlaceProvider provider) async {
    if (!_checkIfValidArea(LatLng(provider.cameraPosition.target.latitude,
        provider.cameraPosition.target.longitude))) {
      PickResult pickResult =
          PickResult(formattedAddress: "We're not there yet!", placeId: "-1");
